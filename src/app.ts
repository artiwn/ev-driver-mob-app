...
function homeScreen() {
    ...
    const stateBattery = appState === 'charging' ? charging.battery : appState === 'completed' ? chargingSummary.endBattery : (activeVehicle?.battery ?? stateMeta[appState].battery);
    const meta = {
        ...stateMeta[appState],
        battery: stateBattery,
        label: atStation ? 'Ready to start' : stateMeta[appState].label,
        eyebrow: atStation ? `At station · Charger ${activeReservation?.charger || reservation.charger || '04'} ready` : appState === 'charging' ? chargingOperationalStatus().detail : stateMeta[appState].eyebrow
    };
    const primary = primaryAction(appState, atStation);
    const scenario = homeScenario;
    const telemetrySource = appState === 'charging' ? 'Session estimate' : appState === 'completed' ? 'Session result' : 'Last reported';
    ...
    const reservationCard = `<button class="info-card reservation-card card-button" data-home-reservation><div class="reservation-icon">${icon('clock')}</div><div><small>${appState === 'reserved' ? 'Active reservation' : 'Next availability'}</small><h3>${appState === 'reserved' ? `${reservationInfo.date}, ${reservationInfo.time}` : 'No reservation'}</h3><p>${appState === 'reserved' ? `${atStation?'Arrived':`Starts in ${activeReservation?.countdownMinutes ?? 18} min`} · Bay ${reservationInfo.bay || 'B-12'} · Charger ${reservationInfo.charger || '04'}` : 'Reserve a charger before arrival'}</p></div><span>${icon('chevron')}</span></button>`;
...
function filteredStations() {
    ...
    return [...result].sort((a,b) => mapSort === 'price' ? a.price-b.price
        ...
        : mapSort === 'availability' ? (b.available-a.available || a.distanceKm-b.distanceKm)
...
function chargerCodeForConnector(connector) {
    const type = String(connector?.type || 'CCS2').toUpperCase().replace(/[^A-Z0-9]/g, '');
    return `VD-${connector?.id || '04'}-${type}`;
}
function connectorFromChargerCode(code) {
    const normalized = String(code || '').trim().toUpperCase().replace(/\s+/g, '');
    return stationConnectorRows(selectedStation).find(connector => chargerCodeForConnector(connector) === normalized) || null;
}
...
function locationScreen() {
    ...
    const connectors = stationConnectorRows(s);
    const firstAvailable = connectors.find(c => c.status === 'available');
    const estimate = Math.round(39 * s.price);
    const connectorList = connectors.map((c, index) => `<button class="charger-row ${c.status==='available' && index===connectors.findIndex(x=>x.status==='available')?'selected ':''}${c.status!=='available'?'disabled ':''}ui-on-dark" ${c.status==='available'?`data-start-with-connector="${c.id}"`:''}><span class="charger-number">${c.id}</span><span><small>${c.type} · ${c.state}</small><strong>${c.power} kW ${c.speed}</strong></span><span><small>${c.price} AMD/kWh</small><strong>${c.status==='available'?'Ready':c.status==='offline'?'Offline':'Busy'}</strong></span></button>`).join('');
...
function reservationManageScreen() {
    ...
    const assigned = r.type==='Any available charger' ? 'Charger assigned on arrival' : `Charger ${r.charger} · Bay ${r.bay}`;
    return simpleHeaderBack('Reservation details',`${r.id} · ${r.status}`, `${reservationMessage?`<div class="ui-feedback ui-feedback--success">${reservationMessage}</div>`:''}<section class="reservation-status-card ui-surface--dark"><div><small>Arrival window</small><h2>${r.date}, ${r.time}</h2><p>${station.name} · ${assigned}</p></div><span class="reservation-countdown"><small>Starts in</small><strong>${r.countdownMinutes ?? 18} min</strong></span></section><section class="grace-card"><div class="grace-ring"><strong>${graceMinutes}</strong><small>min</small></div><div><small>Arrival grace period</small><h2>Your reservation stays protected</h2><p>Confirm arrival before ${addMinutesToClock(r.time,graceMinutes)} or the reservation may become a no-show.</p></div></section><section class="ui-card reservation-detail-list"><div><span>Vehicle</span><strong>${r.vehicle}</strong></div><div><span>Reservation type</span><strong>${r.type}</strong></div><div><span>Expected duration</span><strong>${r.duration} min</strong></div><div><span>Target battery</span><strong>${r.target}%</strong></div><div><span>Reservation fee</span><strong>${reservationFee.toLocaleString()} AMD</strong></div></section><button class="ui-button ui-button--primary ui-button--block" data-confirm-arrival>I'm at the station</button><div class="reservation-action-grid"><button class="ui-button ui-button--secondary" data-modify-reservation>Modify</button><button class="ui-button ui-button--danger" data-cancel-reservation>Cancel reservation</button></div><button class="ui-text-button reservation-demo-link" data-simulate-no-show>Prototype: simulate no-show</button>`, 'reservation-manage-return');
}
function cancelReservationScreen() {
    const r=activeReservation||reservation;
    const station=reservationStation(r);
    const cancellationFee = cancellationReason === 'Charger unavailable' ? 0 : reservationFee;
    const refund = cancellationFee===0 ? reservationFee : 0;
    return simpleHeaderBack('Cancel reservation','Review cancellation conditions', `<section class="ui-card cancel-summary"><small>Reservation ${activeReservation?.id||'Pending'}</small><h2>${station.name}</h2><p>${r.date}, ${r.time} · ${r.type==='Any available charger'?'Charger assigned on arrival':`Charger ${r.charger}`}</p></section><section class="ui-card ui-form"><label><span>Reason for cancellation</span><select id="cancel-reason"><option ${cancellationReason==='Plans changed'?'selected':''}>Plans changed</option><option ${cancellationReason==='Running late'?'selected':''}>Running late</option><option ${cancellationReason==='Found another charger'?'selected':''}>Found another charger</option><option ${cancellationReason==='Charger unavailable'?'selected':''}>Charger unavailable</option></select></label></section><section class="cost-card"><div><span>Reservation fee paid</span><strong>${reservationFee.toLocaleString()} AMD</strong></div><div><span>Refund</span><strong>${refund.toLocaleString()} AMD</strong></div><div class="total"><span>Cancellation fee</span><strong>${cancellationFee.toLocaleString()} AMD</strong></div><small>Free cancellation is available until 30 minutes before the scheduled arrival. Charger unavailability is refunded automatically.</small></section><button class="ui-button ui-button--danger ui-button--block" data-confirm-cancel>Confirm cancellation</button>`, 'reservation-manage');
}
...
function exportSessionCsv(session) {
    ...
    const rows = [
      ...
      ['Charging limit', session.chargingLimit || '—'], ['Energy charge AMD', session.energyCharge ?? session.cost], ['Reservation fee AMD', session.reservationFee ?? 0],
...
function addSystemNotification(title,text,type='success',target='session-detail',actionLabel='Open details',targetId=null){
    ...
    notifications.unshift({id,group:'Today',time:'Just now',title,text,type,icon:type==='payment'?'▭':type==='reserved'?'R':'✓',unread:true,target,actionLabel,targetId});
}
...
function updateReservationRecord(r,status='Confirmed',refund){
    if(!r?.id) return;
    const station=reservationStation(r);
    const vehicle=vehicles.find(v=>v.name===r.vehicle);
    const payload={id:r.id,place:station?.name||selectedStation.name,date:`${r.date}, ${r.time}`,status,bay:r.bay,charger:r.type==='Any available charger'?'Any':r.charger,fee:reservationFee,vehicle:vehicle?`${vehicle.name} · ${vehicle.plate}`:r.vehicle};
...
function finalizeChargingSession(status = 'Completed', reason = 'Target reached'){
    ...
    const hadReservation=Boolean(activeReservation);
    ...
    sessions.unshift({id:sessionId,place:station.name,address:station.address,date:now,started:context.started,ended:'12:25',energy:chargingSummary.energy,cost:amount,status,reason,charger:connector?.id||startCharge.connector,connector:connector?.type||station.connector||'CCS2',vehicle:`${vehicle.name} · ${vehicle.plate}`,startBattery:chargingSummary.startBattery,endBattery:chargingSummary.endBattery,averagePower,peakPower:`${charging.power} kW`,chargingLimit:chargingLimitLabel(activeChargingSession?.limit || chargeLimit),paymentId,receipt:receiptId,energyCharge:amount,reservationFee:hadReservation?reservationFee:0,parkingFee:0,idleFee:0,vat:completed?Math.round(amount/6):0,paymentMethod});
    ...
    if(activeReservation) updateReservationRecord(activeReservation,'Completed');
    activeReservation=null; activeChargingSession=null; startCharge.error='';
    if(completed){
      ...
      appState='idle'; startCharge.stage='interrupted';
...
function cancelActiveReservation(){
    ...
    const refund=cancellationReason === 'Charger unavailable' ? reservationFee : 0;
    let refundPaymentId='';
    if(refund>0){
      walletBalance+=refund;
      refundPaymentId=nextRecordId('RF',activityPayments);
      activityPayments.unshift({id:refundPaymentId,date:'Just now',title:'Reservation refund',method:'Wallet balance',amount:refund,status:'Refunded',reservationId:current.id});
      latestPaymentId=refundPaymentId;
    }
    updateReservationRecord(current,'Cancelled',refund);
    addSystemNotification('Reservation cancelled',refund>0?`${refund.toLocaleString()} AMD returned to your Wallet.`:'Reservation cancelled. The reservation fee is non-refundable at this stage.',refund>0?'payment':'reserved',refund>0?'payment-detail':'activity',refund>0?'View refund':'View reservations',refundPaymentId||current.id);
...
function activityScreen(){
    ...
    if(activitySection==='sessions'){
      const rows=sessions.filter(s=>activityMatches(`${s.place} ${s.id} ${s.status} ${s.vehicle} ${s.charger} ${s.connector}`));
...
function reportProblemScreen(){
    ...
    if(reportSubmitted) return simpleHeaderBack('Problem reported','Service request created', `<section class="report-success"><span>✓</span><h2>We received your report</h2><p>Ticket VD-M-2048 was sent to the operations team. Charger ${chargerId} will be checked remotely first.</p><div class="report-ticket"><div><small>Priority</small><strong>High</strong></div><div><small>Expected update</small><strong>Within 5 min</strong></div></div></section><button class="primary-action compact" data-return-report><span class="primary-icon">${icon('history')}</span><span><small>Return to previous screen</small><strong>Back to session</strong></span><span>${icon('chevron')}</span></button>`, 'report-return');
...
function runQAScenario(id){
    ...
    if(id==='charge'){ activeTab='charge'; startCharge={code:'VD-04-CCS2',connector:'04',payment:paymentMethodLabel(defaultPaymentMethod()),preauth:5000,accepted:true,error:'',stage:'ready'}; screen='charge-start'; return; }
...
function render() {
    ...
    app.querySelector('[data-qa-reset]')?.addEventListener('click',()=>{qaCompleted.clear();qaMessage='QA checklist reset.';render();});
        app.querySelector('[data-reset-demo]')?.addEventListener('click', () => { appState='idle'; activeChargingSession=null; homeScenario='normal'; homeVehicleMenuOpen=false; showFavoritesOnly=false; recentMapSearches=['Northern Avenue','CCS2']; charging={ battery:64,target:90,power:142,energy:18.6,cost:2232,minutes:17,remaining:26,speed:'Maximum',paused:false }; chargeLimit={type:'battery',battery:90,energy:30,cost:3500,time:45}; chargeLimitReturnScreen='tariff-review'; chargeLimitDraft=null; chargingSummary={ startBattery:64,endBattery:90,energy:31.8,cost:3816,duration:43,status:'Completed',reason:'Target reached' }; onboardingComplete=false; startCharge={ code:'VD-04-CCS2',connector:'04',payment:paymentMethodLabel(defaultPaymentMethod()),preauth:5000,accepted:true,error:'',stage:'idle' }; render(); });
...
app.querySelector('[data-auto-topup-toggle]')?.addEventListener('change', e=>{autoTopUp.enabled=e.target.checked;render();});
    ...
    app.querySelectorAll('[data-report-problem]').forEach(b => b.addEventListener('click', () => { reportSubmitted = false; reportReturn={screen,tab:activeTab}; screen = 'report-problem'; render(); }));
    ...
    app.querySelectorAll('[data-notification-open]').forEach(b => b.addEventListener('click', () => { const n=notifications.find(x=>x.id===Number(b.dataset.notificationOpen));if(!n)return;n.unread=false;const origin={screen,tab:activeTab};if(n.target==='session-detail'){sessionDetailReturn=origin;selectedActivityId=n.targetId||latestCompletedSessionId;activeTab='activity';screen='session-detail';}else if(n.target==='payment-detail'){paymentDetailReturn=origin;selectedActivityId=n.targetId||latestPaymentId;activeTab='activity';screen='payment-detail';}else if(n.target==='reservation-manage'){reservationManageReturn=origin;restoreReservationStation();appState='reserved';screen='reservation-manage';}else if(n.target==='activity'){activeTab='activity';activitySection='reservations';screen='activity';}else if(n.target==='charging-summary'){activeTab='charge';screen='charging-summary';}else if(n.target==='location'){locationReturn=origin;selectedStation=stations.find(s=>s.id===Number(n.targetId))||stations[1]||selectedStation;activeTab='map';screen='location';}else if(n.target==='wallet'){walletReturn=origin;activeTab='account';screen='wallet';}render(); }));
    ...
    app.querySelectorAll('[data-state]').forEach(b => b.addEventListener('click', () => { appState = b.dataset.state; if(appState==='charging'&&!activeChargingSession) captureActiveChargingSession(); if(appState==='charging') recalculateChargingRemaining(); if(appState!=='charging') activeChargingSession=null; render(); }));
    ...
    app.querySelector('[data-primary]')?.addEventListener('click', e => {
        ...
        else if (action === 'navigate') { restoreReservationStation(); navigationState={source:'reservation',started:false,progress:0,arrived:false,arrivalConfirmed:false}; screen='navigation-preview'; }
        else if (action === 'start-reserved-charge') { restoreReservationStation(); startCharge.connector=activeReservation?.charger||reservation.charger||'04'; activeTab='charge'; screen='charger-check'; }
    ...
    app.querySelector('[data-arrival-confirm]')?.addEventListener('click',()=>{navigationState.arrivalConfirmed=true;reservationMessage='Arrival confirmed · '+(activeReservation?.type==='Any available charger'?'Your reservation is active.':'Charger '+(activeReservation?.charger||reservation.charger)+' is protected for you.');navigationMessage=reservationMessage;render();});
    app.querySelector('[data-arrival-start-charge]')?.addEventListener('click',()=>{restoreReservationStation();startCharge.connector=activeReservation?.charger||reservation.charger||'04';activeTab='charge';screen='charger-check';render();});
    ...
    app.querySelector('[data-home-reservation]')?.addEventListener('click',()=>{ if(appState==='reserved'){reservationManageReturn={screen,tab:activeTab};restoreReservationStation();screen='reservation-manage';} else {reservationReturn={screen,tab:activeTab};reservationMode='create';reservationStep=1;screen='reservation';} render();});
    ...
    app.querySelector('[data-reserve]')?.addEventListener('click', () => { reservationReturn={screen,tab:activeTab}; reservationMode='create'; reservationStep = 1; reservationMessage=''; screen = 'reservation'; render(); });
    ...
    app.querySelector('[data-confirm-reservation]')?.addEventListener('click', () => { if(!reservationTermsAccepted){reservationMessage='Accept the reservation conditions to continue.';render();return;} const editing=reservationMode==='edit'&&activeReservation; const rid=editing?activeReservation.id:nextRecordId('VD-RS',activityReservations); activeReservation={...(editing?activeReservation:{}),...reservation,id:rid,status:'Confirmed',stationId:selectedStation.id,stationName:selectedStation.name,stationAddress:selectedStation.address,countdownMinutes:activeReservation?.countdownMinutes ?? 18}; updateReservationRecord(activeReservation,'Confirmed'); addSystemNotification(editing?'Reservation updated':'Reservation confirmed',`${selectedStation.name} · ${reservation.date}, ${reservation.time}`,'reserved','reservation-manage','Manage reservation',rid); appState='reserved'; reservationMessage=''; screen = 'reservation-success'; render(); });
    ...
    app.querySelectorAll('[data-modify-reservation]').forEach(b=>b.addEventListener('click',()=>{reservationMode='edit';if(activeReservation){restoreReservationStation();reservation={...reservation,...activeReservation};}reservationStep=1;reservationMessage='';screen='reservation';render();}));
    ...
    app.querySelector('[data-confirm-cancel]')?.addEventListener('click',()=>{const refund=cancelActiveReservation();reservationMessage=refund?`Reservation cancelled. ${refund.toLocaleString()} AMD returned to your Wallet.`:'Reservation cancelled.';activeTab='home';screen='home';render();});
    ...
    app.querySelectorAll('[data-use-alternative]').forEach(b=>b.addEventListener('click',()=>{reservationReturn={screen,tab:activeTab};selectedStation=stations.find(x=>x.id===Number(b.dataset.useAlternative))||stations[0];waitingListJoined=false;reservationMode='create';reservationStep=1;activeTab='map';screen='reservation';render();}));
    app.querySelector('[data-leave-waiting-list]')?.addEventListener('click',()=>{waitingListJoined=false;screen='location';render();});
    app.querySelectorAll('[data-scan-charger]').forEach(b=>b.addEventListener('click',()=>{activeTab='charge';scannerFlashlight=false;screen='charge-scan';chargeStartMessage='';render();}));
    app.querySelector('[data-begin-scan]')?.addEventListener('click',()=>{scannerFlashlight=false;screen='charge-scan';render();});
    ...
    app.querySelector('[data-check-code]')?.addEventListener('click',()=>{const code=document.querySelector('#charger-code')?.value?.trim();if(!code){chargeStartMessage='Enter the charger code.';render();return;}const connector=connectorFromChargerCode(code);if(!connector){chargeStartMessage='Charger code was not recognized for the selected station. Check the label and try again.';render();return;}startCharge.code=chargerCodeForConnector(connector);startCharge.connector=connector.id;startCharge.stage='checking';screen='charger-check';chargeStartMessage='';render();});
    app.querySelector('[data-simulate-scan]')?.addEventListener('click',()=>{const activeVehicle=vehicles.find(v=>v.active)||vehicles[0];const connector=stationConnectorRows(selectedStation).find(c=>c.status==='available' && (!activeVehicle?.connector || c.type===activeVehicle.connector)) || stationConnectorRows(selectedStation)[0];scannerFlashlight=false;startCharge.code=chargerCodeForConnector(connector);startCharge.connector=connector.id;startCharge.stage='checking';screen='charger-check';chargeStartMessage='';render();});
    app.querySelector('[data-use-recent-charger]')?.addEventListener('click',e=>{const stationId=Number(e.currentTarget.dataset.stationId);const connectorId=e.currentTarget.dataset.connectorId;const station=stations.find(item=>item.id===stationId);if(!station)return;selectedStation=station;startCharge.connector=connectorId||stationConnectorRows(station)[0]?.id||'04';startCharge.code=chargerCodeForConnector(selectedStationConnector());startCharge.stage='checking';chargeStartMessage='';screen='charger-check';render();});
    app.querySelector('[data-use-reserved-charger]')?.addEventListener('click',()=>{restoreReservationStation();startCharge.connector=activeReservation?.charger||reservation.charger||'04';startCharge.code=chargerCodeForConnector(selectedStationConnector());startCharge.stage='checking';screen='charger-check';chargeStartMessage='';render();});
    app.querySelectorAll('[data-start-with-connector]').forEach(b=>b.addEventListener('click',()=>{startCharge.connector=b.dataset.startWithConnector;startCharge.code=chargerCodeForConnector(selectedStationConnector());startCharge.stage='checking';activeTab='charge';screen='charger-check';chargeStartMessage='';render();}));
    ...
    app.querySelector('[data-summary-home]')?.addEventListener('click', () => { appState = chargingSummary.status==='Completed' ? 'completed' : 'idle'; activeTab = 'home'; screen = 'home'; render(); });
